...
public class ExchangeRatesClient implements ExchangeRateClient {
    ...
    private String buildUrl(final String symbols) {
        if (symbols == null) {
            return String.format("%s%s?access_key=%s", exchangeratesUrl, ENDPOINT, exchangeratesAccessKey);
        }
        return String.format("%s%s?access_key=%s&symbols=%s", exchangeratesUrl, ENDPOINT, exchangeratesAccessKey, symbols);
    }
}
